...
public class HomePageStep extends HomePage {
    ...
    public HomePageStep checking_crossed_out_price(){
        for (int i = 0; i < sale_slider_size(); i++){
            soft.assertTrue(sale_slider.$(".sc-bdd54c0a-0", i).$(".sc-bdd54c0a-11").is(Condition.enabled),"ყველა ნივთს უნდა ქონდეს დაკლებული ფასი");
            soft.assertTrue(sale_slider.$(".sc-bdd54c0a-0", i).$(".sc-bdd54c0a-32").is(Condition.enabled),"ყველა ნივთს უნდა ქონდეს რომელიმე აქცის ნიშანი");
            soft.assertAll();
        }
        return this;
    }
